...
const MyAppointments = () => {
  ...
  const appointmentRazorpay = async (appointmentId) => {
    console.log("Pay button clicked for appointment ID: ", appointmentId); // Add this to check if the function is triggered
    try {
        ...
        <div>
          {appointments.slice(0,3).map((item,index)=>(
            <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
                ...
                {item.payment && !item.cancelled && !item.isCompleted && <button className='sm:min-w-48 py-2 border rounded text-stone-500 bg-indigo-50'>Paid</button>}
