  ...
  const bookAppointment = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        let updatedSlots = { ...docInfo.slots_booked };
        ...
        // Update docInfo
        setDocInfo((prev) => ({ ...prev, slots_booked: updatedSlots }));

        // Remove the booked slot from available slots
        const updatedDocSlots = docSlots.map((slots, index) =>
          index === slotIndex
            ? slots.filter((slot) => slot.time !== slotTime)
            : slots
        );

        setDocSlots(updatedDocSlots); // Update available slots

        getDoctorsData(); // Refresh doctor data from backend
